...
/*
 ...
 @dev: Device structure for the led_patterns component. This device
        struct is embedded in the led_patterns' platform device struct
 @attr: Unused
...
/*
 ...
 @dev: Device structure for the led_patterns component. This device struct
        is embedded in the led_patterns' device struct.
 @attr: Unused
 @buf: Buffer that gets returned to user-space.
...
static ssize_t hps_led_control_show(struct device *dev, struct device_attribute *attr, char *buf){
    ...
    // Get the private led_patterns data out of the dev strut
    struct led_patterns_dev *priv = dev_get_drvdata(dev);
...
static const struct of_device_id led_patterns_of_match[]={
    { .compatible = "Allick,led_patterns", },
    {}
};
...
/*
 ...
 Return: On success, the number of bytes written is returned and
         the offset @offset is advanced by this number. On error, 
...
/*
 ...
 Return: On success, the number of bytes written is returned and the 
         offset @offset is advanced by this number. On error, a negative
...
static ssize_t led_patterns_write(struct file *file, const char __user *buf, size_t count, loff_t *offset){
    ...
    if((*offset % 0x4) != 0){
        pr_warn("led_patterns_write: unaligned address\n");
        return -EFAULT;
    }
...
/*
 ...
 @owner: The led_patterns driver owns the file operations; this ensures
         that the driver can't be removed while the character
...
/*
 ...
 @pdev: Platform device structure associated with our led patterns 
        device; pdev is automatically created by the driver core based
        upon our led patterns device tree node

 When a device that is compatible with this led patterns driver is found,
 the driver's probe function is called. This probe function gets called
 by the kernel when an led_patterns device is found in the device tree.
*/
static int led_patterns_probe(struct platform_device *pdev)
{
    ...
    */
    ...
    // Initialize the misc device paramaters
...
/*
 ...
 This function is called when an led patterns device is removed or the 
 driver is removed
*/
...
/*
 ...
 @probe: Function that is called when a device is found
 @remove: Function that is called when a device is removed
...
MODULE_DESCRIPTION("led_patterns_driver");
